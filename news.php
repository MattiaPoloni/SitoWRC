...
<head>
	...
	<body>
		<?php
			...
			$query = "SELECT titolo, descrizione, fonte, indirizzo FROM Notizia ORDER BY id DESC;";
			$ris = $connessione->query($query);
			if(!$ris) {
                echo "Errore Database";
            } else {
                while($row = $ris->fetch_array(MYSQLI_NUM)) {
						$link = "<a href='$row[3]'>Continea a leggere</a>";
                        echo "<p><h2>$row[0]</h2> Fonte: $row[2]<br/>$row[1] $link</p>";
				}
			}
			$connessione->close();
